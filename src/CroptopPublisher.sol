...
contract CroptopPublisher {
     ...
     */
    mapping(uint256 => mapping(address => mapping(uint256 => bool))) public allowedCategoryFor;
     ...
     * _category The category.
     */
    mapping(uint256 => mapping(address => mapping(uint256 => uint256))) public minPostPriceFor;
     ...
     * _category The category.
     */
    mapping(uint256 => mapping(address => mapping(uint256 => uint256))) public minPostQuantityFor;
    ...
    function collect(uint256 _projectId, Post[] memory _posts, address _beneficiary) external payable {
        ...
        uint256 _startingTierId =
            IJBTiered721Delegate(_metadata.dataSource).store().maxTierIdOf(_metadata.dataSource) + 1;
        ...
        // Scoped section to prevent stack too deep.
        {
            ...
            for (uint256 _i; _i < _numberOfPosts;) {
                ...
                if (_tierIdsToMint[_i] == 0) {
                    ...
                    if (!allowedCategoryFor[_projectId][_metadata.dataSource][_post.category]) {
                        revert UNAUTHORIZED_CATEGORY();
                    }

                    // Make sure the price being paid for the post is at least the allowed minimum price.
                    if (_post.price < minPostPriceFor[_projectId][_metadata.dataSource][_post.category]) {
                    ...
                    if (_post.quantity < minPostQuantityFor[_projectId][_metadata.dataSource][_post.category]) {
    ...
    function configure(uint256 _projectId, AllowedPost[] memory _allowedPosts) external {
        ...
        for (uint256 _i; _i < _numberOfAllowedPosts;) {
            ...
            allowedCategoryFor[_projectId][_allowedPost.nft][_allowedPost.category] =
                !allowedCategoryFor[_projectId][_allowedPost.nft][_allowedPost.category];

            // Set the minimum price for posts to the specific category.
            minPostPriceFor[_projectId][_allowedPost.nft][_allowedPost.category] = _allowedPost.minimumPrice;

            // Set the minimum quantity that can be minted for the specific category.
            minPostQuantityFor[_projectId][_allowedPost.nft][_allowedPost.category] = _allowedPost.minimumQuantity;
